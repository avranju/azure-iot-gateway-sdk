...
static MODULE_LIBRARY_HANDLE DynamicModuleLoader_Load(const MODULE_LOADER* loader, const void* entrypoint)
{
    ...
    if (loader == NULL || entrypoint == NULL)
    {
        result = NULL;
        LogError(
            "DynamicModuleLoader_Load() - invalid input - loader = %p, entrypoint = %p",
            loader, entrypoint
        );
    }
    else
    {
        ...
        {
            result = NULL;
            LogError("DynamicModuleLoader_Load() - loader->type is not NATIVE");
        }
        else
        {
            ...
            else
            {
                ...
                if (result == NULL)
                {
                    ...
                    LogError("DynamicModuleLoader_Load() - malloc(sizeof(DYNAMIC_MODULE_HANDLE_DATA)) failed");
                }
                else
                {
                    /* load the DLL */
